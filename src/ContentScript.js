...
export class ContentScript extends Component {
  ...
  componentDidMount = () => {
    ...
    setTimeout(() => {
      ...
      this.contentConnection.onicecandidate = (e) => {
        if(e.candidate)
        {
          this.port.postMessage({ message: "Content Ice-Candidate", candidate: e.candidate });
        }
      }
      this.contentConnection.createOffer({ offerToReceiveVideo: true })
      .then((offer) => {
        this.contentConnection.setLocalDescription(offer);
      })
      .then(() => {
        this.port.postMessage({ 
          message: "Content-SDP", 
          sdp: this.contentConnection.localDescription
        })
      })
    }, 5000)
